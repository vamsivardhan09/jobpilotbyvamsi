...
import {
  ...
  FileText, TrendingUp, AlertTriangle, ChevronRight, Settings
} from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [profileRes, resumeRes, matchRes, skillsRes] = await Promise.all([
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Skills Tracked", value: skills.length, icon: TrendingUp, color: "text-warning" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 10 }}
